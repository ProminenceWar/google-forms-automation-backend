...
- Servicio central para generar, validar, rotar y revocar tokens JWT (access + refresh).

Funciones principales

- `generateTokenPair(userId, additionalClaims = {})` -> Promise<Object>
  - Genera accessToken y refreshToken, guarda el refresh en el usuario y devuelve ambos tokens y expiraciones.
- `refreshAccessToken(refreshToken, options = {})` -> Promise<Object>
  - Verifica el refresh token, valida que exista en BD, rota (opcional) y devuelve nuevos tokens.
- `revokeRefreshToken(userId, refreshToken)` -> Promise<boolean>
  - Elimina un refresh token específico del usuario.
- `revokeAllRefreshTokens(userId)` -> Promise<boolean>
  - Revoca (elimina) todos los refresh tokens de un usuario.
- `validateAccessToken(accessToken)` -> Promise<Object>
  - Verifica y decodifica el access token sin renovarlo.
- `cleanupExpiredTokens()` -> Promise<number>
  - Limpia tokens expirados de todos los usuarios (tarea de mantenimiento).
- `getTokenInfo(userId)` -> Promise<Object>
  - Devuelve información y metadatos de tokens activos del usuario.
